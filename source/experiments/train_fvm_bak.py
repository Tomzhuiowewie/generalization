...
class PINN(nn.Module):
    ...
    def encode_conditions(self, ic, bc):
        """编码一个 case 的初始条件和边界条件，供同一步内的多次预测复用。"""
        iz, iq = ic.chunk(2, -1)
        bq, bz = bc.chunk(2, -1)
        ic_code = self.ic(torch.cat((
            (iz - self.z_mean) / self.z_std,
        ...
        z_global, z_tokens = self.z_encoder(
            ((bz - self.z_mean) / self.z_std)[:1]
        )
        return ic_code, q_global, q_tokens, z_global, z_tokens
    ...
    def forward(self, x, t, ic, bc, geo, mask, bed, geo_weight=None, condition_cache=None):
        ...
        ic_code, q_global, q_tokens, z_global, z_tokens = condition_cache
        ic_code = ic_code.expand(len(x), -1)
...
def pinn_losses(model, cases, points):
    ...
    for case in cases:
        ...
        z0, q0 = case["ic"].chunk(2)    # 初始水位、初始流量
        bq, bz = case["bc"].chunk(2)    # 边界流量、边界水位
        condition_cache = model.encode_conditions(case["ic"][None], case["bc"][None])
        ...
        z, q = model(
            ...
            case["bed"][:, None], condition_cache=condition_cache
        )
        ...
        for section, z_target, q_target in boundaries:
            zp, qp = model(
                ...
                case["bed"][section].expand(points, 1), condition_cache=condition_cache
            )
        ...
        face_x = torch.cat((xm, xm, xl, xr), 0)
        face_t = torch.cat((tb, tt, tm, tm), 0)
        _, face_q, face_area, _ = state(model, case, face_x, face_t, condition_cache)
        qb, qt, ql, qr = face_q.chunk(4, 0)
        ab, at, al, ar = face_area.chunk(4, 0)
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == epochs:
            ...
            row.update(train_z_error=train_z, train_q_error=train_q,
                       z_error=z_error, q_error=q_error, score=max(z_error, q_error))

            if row["score"] < best:
                best = row["score"]
                state = {key: value.detach().cpu() for key, value in model.state_dict().items()}
                torch.save({"model_state_dict": state, "epoch": epoch, "validation": row}, output / "best.pt")
            message += (f" train_z={train_z:.4f}% train_q={train_q:.4f}%"
                        f" val_z={z_error:.4f}% val_q={q_error:.4f}% best={best:.4f}%")
